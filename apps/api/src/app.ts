...
import { deriveChunkPlan } from '@big-upload/upload-core';
...
export async function buildApp(deps:AppDependencies={}):Promise<BuiltApp>{
  ...
  const ack=async(request:FastifyRequest<{Params:{id:string};Body:Static<typeof AckPartsSchema>}>)=>{const value=await ownedUpload(request,request.params.id);if(!activeStates.has(value.status))throw new ApiError(409,'INVALID_UPLOAD_STATE','Upload cannot acknowledge parts');const existing=await db.listParts(value.id);let uploaded=existing.reduce((sum,part)=>sum+part.size,0);let expected=existing.length+1;for(const part of request.body.parts){const prior=existing.find(item=>item.partNumber===part.partNumber);if(prior){if(prior.size!==part.size)throw new ApiError(400,'INVALID_PART_SIZE','Part does not match the acknowledged upload');continue}if(part.partNumber!==expected++||part.size>value.byteSize-uploaded)throw new ApiError(400,'INVALID_PART_SIZE','Part does not match the adaptive upload sequence');uploaded+=part.size;await db.upsertPart(value.id,part.partNumber,part.etag.replaceAll('"',''),part.size)}if(uploaded===value.byteSize)await db.setUploadTotalParts(value.id,expected-1);if(value.status==='INITIATED')await db.setUploadStatus(value.id,'UPLOADING');telemetry.event('upload.part_acknowledged',{count:request.body.parts.length});return{uploadedParts:await db.listParts(value.id)}};
